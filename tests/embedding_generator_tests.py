...
class TestEmbeddingGenerator(unittest.TestCase):
    ...
    def test_multiple_text_cols(self):
        ...
        artifacts = {}
        data, artifacts = self.embedding_generator.run(data=self.data.data,
                                                       artifacts=artifacts)

        # Compare whether the two columns have the same values.
        self.assertEqual(602, len(artifacts["id_to_token"]),
                         'Incorrect.')
    ...
    def test_write_data(self):
        ...
        for test_format in test_formats:
            ...
            self.embedding_generator.write_data(write_path=test_data_dir,
                                                data=data)
            self.embedding_generator.write_artifacts(write_path=test_data_dir,
                                                     artifacts=artifacts)
            test_data_contents = os.listdir(test_data_dir)
            result = False
            for test_file in test_data_contents:
                test_file_match = re.search(
                    f'CPUEmbeddingGeneratorData.{test_format}', test_file)
                if test_file_match:
                    result = True
            ...
            embedding_matrix_path = os.path.join(
                test_data_dir, 'embedding_matrix.npy')
            embedding_matrix_result = os.path.isfile(embedding_matrix_path)
            os.remove(embedding_matrix_path)
            self.assertTrue(embedding_matrix_result,
                            "Embedding matrix was not written.")
